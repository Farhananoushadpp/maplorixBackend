...
export const verifyRecaptcha = async (req, res, next) => {
  try {
    ...
    if (process.env.NODE_ENV !== "production" && process.env.ENABLE_RECAPTCHA_IN_DEV !== "true") {
      return next();
    }

    // Get reCAPTCHA token from request body
    const captchaToken = req.body?.recaptchaToken || req.body?.gRecaptchaResponse;
    
    if (!captchaToken || captchaToken === "undefined" || captchaToken === "") {
      return res.status(400).json({
        success: false,
        error: "Validation Error",
        message: "reCAPTCHA verification is required. Please complete the reCAPTCHA challenge.",
      });
    }
    ...
    if (!recaptchaSecret || recaptchaSecret === "your_recaptcha_secret_key_here") {
      ...
      return res.status(500).json({
        ...
        message: "reCAPTCHA is not properly configured. Please contact support.",
      });
    }
    ...
    if (recaptchaData.score !== undefined) {
      ...
      if (recaptchaData.score < minScore) {
        console.error(`reCAPTCHA score too low: ${recaptchaData.score} < ${minScore}`);
        return res.status(400).json({
          success: false,
          error: "Validation Error",
          message: "reCAPTCHA verification failed. Please try again.",
