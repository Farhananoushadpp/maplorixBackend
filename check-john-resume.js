...
const checkJohnResume = async () => {
  try {
    // Connect to MongoDB
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/maplorix');
    console.log('🔗 Connected to MongoDB');

    // Find the most recent application (john)
    ...
    if (application) {
      console.log('📋 Latest application (john):');
      ...
      if (application.resume && application.resume.filename) {
        ...
        if (fs.existsSync(application.resume.path)) {
          ...
          console.log('🎉 RESUME UPLOAD AND STORAGE WORKING PERFECTLY!');
        } else {
